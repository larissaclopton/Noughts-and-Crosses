...
class WebService {
    ...
    func createMutableRequest(url:NSURL!,method:String!,parameters:Dictionary<String, String>?) -> Request  {
        ...
        // build request
        let headers = ["access-Token":UserController.sharedInstance.currentlyLoggedIn!.token, "client": UserController.sharedInstance.currentlyLoggedIn!.client, "uid":UserController.sharedInstance.currentlyLoggedIn!.email, "token-type":"bearer"]
